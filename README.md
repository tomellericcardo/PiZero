...
* Sistema operativo Raspbian (testato solo con stretch lite)
...
```
sudo apt-get install python-pip python-picamera imagemagick gpac libav-tools -y
...
Il server web si metterà in ascolto alla porta `80`, per cui per raggiungere l'applicazione web basterà aprire il browser di un dispositivo connesso alla stessa rete del Raspberry Pi ed inserirne l'indirizzo IP.
